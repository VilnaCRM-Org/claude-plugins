...
description: "Use when preparing saved-plan promotion, deployment health gates or release rollback. Use incident-response for broader incident triage and state-migration for backend ownership changes."
---
...
5. On failure, preserve evidence, determine partial application, inspect service
   ...
   recovery have separate compatibility checks. If a reviewed configured
   rollback entry and prior authorization already cover the exact ephemeral
   environment, release and failure trigger, prescribe that entry with its
   exact argv and reuse that scope. Preserve the failed smoke evidence, require
   rollback output and post-rollback health checks, then diagnose the source fix.
   Do not replace required recovery with only a future fix/redeploy or ask for
   repeated approval of the same covered action. Keep production or wider-scope
   recovery blocked when not authorized. The generic helper has no rollback
   subcommand; use the reviewed recovery runbook/CI entry within its own scope.
   In simulation, propose this recovery without claiming it already executed.
