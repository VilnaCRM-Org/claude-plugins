...
| --- | --- | --- |
| RL-P01 | PASS verdict on first iteration | PENDING |
| RL-P02 | FAIL then PASS | PENDING |
| RL-P03 | PASS exactly at `--max-iterations` boundary | PENDING |
| RL-P04 | `--max-iterations 1` with immediate PASS | PENDING |
| RL-P05 | JSON with extra fields (canonical CLI shape) | PENDING |
| RL-P08 | `REVIEW_PROMPT` env override | PENDING |
| RL-P09 | space-separated `--agents "codex claude"` | PENDING |
| RL-P10 | profile matrix mixed `[codex, claude]` | PENDING |
| RL-N01 | perpetual FAIL hits default cap (NFR-6) | PENDING |
| RL-N02 | well-formed FAIL is NOT retried | PENDING |
| RL-N03 | verdict missing entirely | PENDING |
| RL-N04 | verdict present but not last line | PENDING |
| RL-N05 | `is_error:true` with exit 0 | PENDING |
| RL-N06 | claude exit code 1 | PENDING |
| RL-N07 | claude exit code 124 | PENDING |
| RL-N08 | empty result string | PENDING |
| RL-N09 | `--max-iterations 0` | PENDING |
| RL-N12 | all agents unsupported via `--agents codex` | PENDING |
| RL-E01 | `is_error:true` then clean PASS on retry | PENDING |
| RL-E02 | garbage (non-JSON) then PASS on retry | PENDING |

### Round-2 new cases

| ID | Case | Setup / command | Expected | Result |
| --- | --- | --- | --- | --- |
| RL2-01 | malformed YAML profile | `.claude/php-sdlc.yml` with broken syntax (`[unclosed`), no `--agents`; `PASS_JSON` | no crash; degrades to default agent claude; exit 0; 1 call | PENDING |
| RL2-02 | profile `review.ai_review_agents` is a scalar, not a list | `review:\n  ai_review_agents: claude`; `PASS_JSON` | no crash; either uses claude or defaults to claude; exit 0 | PENDING |
| RL2-03 | `REVIEW_PROMPT` injection payload | `REVIEW_PROMPT='$(touch HACKED) \`touch HACKED2\` ; rm -rf x'` + `PASS_JSON` | no file created, no command executed; payload passed verbatim as one argv to claude; exit 0 | PENDING |
| RL2-04 | jq absent — python3 JSON fallback, PASS | restricted PATH without jq (bash/awk/dirname/env/python3 + stub only); `PASS_JSON` | exit 0; PASS on iteration 1; 1 call | PENDING |
| RL2-05 | jq absent — python3 fallback, `is_error:true` | same restricted PATH; is_error JSON; max 1 | exit 1; `is_error` warn; one retry; 2 calls | PENDING |
| RL2-06 | CRLF line endings in result | `{"result":"ok\r\nAI_REVIEW_VERDICT: PASS\r"}`; max 1 | trailing CR breaks the exact verdict match → fail closed (exit 1, contract warn, no retry, 1 call); never a silent pass | PENDING |
| RL2-07 | `.result` is JSON `null` | `{"result":null}`; max 1 | transport path (no `.result`); one retry; exit 1; 2 calls | PENDING |
| RL2-08 | `is_error` is string `"true"`, not bool | `{"is_error":"true","result":"ok\nAI_REVIEW_VERDICT: PASS"}`; max 1 | strict bool compare → not a transport error; verdict parsed; exit 0; 1 call (record) | PENDING |
| RL2-09 | `--diff-base` flows into default prompt | `--diff-base develop` + `PASS_JSON`, no `REVIEW_PROMPT` | call log contains `git diff against develop`; exit 0 | PENDING |
| RL2-10a | `--max-iterations 007` (leading zero) | run with `--max-iterations 007` | exit 1; `must be a positive integer`; 0 calls | PENDING |
| RL2-10b | large `--max-iterations 100`, immediate PASS | `--max-iterations 100` + `PASS_JSON` | exit 0; exactly 1 call (no useless looping) | PENDING |
| RL2-11 | mixed separators `--agents "codex, claude"` | comma+space in one token; `PASS_JSON` | codex warn+skip; empty token dropped; exit 0; 1 call | PENDING |
| RL2-12 | trailing garbage after JSON on stdout | stub prints `PASS_JSON` then a bare log line; max 1 | record behavior; must end deterministically (PASS via salvaged `.result`, or transport retry); no crash | PENDING |
| RL2-13 | multi-line `REVIEW_PROMPT` override | 3-line prompt via env + `PASS_JSON` | full prompt incl. newlines reaches claude argv; exit 0 | PENDING |
| RL2-14 | python YAML fallback for profile matrix | `SDLC_FORCE_PYTHON_YAML=1`, profile `[codex, claude]`; `PASS_JSON` | codex warn+skip; claude PASS; exit 0; 1 call (parity with RL-P10) | PENDING |
