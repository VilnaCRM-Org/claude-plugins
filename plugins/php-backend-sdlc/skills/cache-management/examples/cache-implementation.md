...
The `Customer` entity is illustrative. Namespaces follow the PSR-4 prefix of `architecture.source_root`; `{Context}` is one of `architecture.bounded_contexts`; `Shared` stands for `architecture.shared_context`. The inner repository's concrete name derives from `persistence.mapper`/`persistence.engine` (shown here as `{Persistence}CustomerRepository` — see "Repository naming" in [SKILL.md](../SKILL.md)).
...
│  └─ inner.save(customer)  // delegates to persistence repo  │
...
| CachedCustomerRepository            | `<source_root>/{Context}/Infrastructure/Repository/CachedCustomerRepository.php`                     |
| {Persistence}CustomerRepository     | `<source_root>/{Context}/Infrastructure/Repository/{Persistence}CustomerRepository.php`              |
| CustomerCreatedCacheInvalidationSub | `<source_root>/{Context}/Application/EventSubscriber/CustomerCreatedCacheInvalidationSubscriber.php` |
| CustomerUpdatedCacheInvalidationSub | `<source_root>/{Context}/Application/EventSubscriber/CustomerUpdatedCacheInvalidationSubscriber.php` |
| CustomerDeletedCacheInvalidationSub | `<source_root>/{Context}/Application/EventSubscriber/CustomerDeletedCacheInvalidationSubscriber.php` |
...
**Location**: `<source_root>/{Context}/Infrastructure/Repository/CachedCustomerRepository.php`
...
**Location**: `<source_root>/{Context}/Application/EventSubscriber/CustomerUpdatedCacheInvalidationSubscriber.php`
...
use App\Customer\Domain\Event\CustomerUpdatedEvent;
...
`{Persistence}CustomerRepository` stands for the concrete inner repository named per `persistence.mapper`/`persistence.engine` — substitute the actual class name from the repository.
...
services:
  ...
  App\Customer\Application\EventSubscriber\CustomerCreatedCacheInvalidationSubscriber:
    arguments:
      $cache: '@cache.customer'

  App\Customer\Application\EventSubscriber\CustomerUpdatedCacheInvalidationSubscriber:
    arguments:
      $cache: '@cache.customer'

  App\Customer\Application\EventSubscriber\CustomerDeletedCacheInvalidationSubscriber:
    arguments:
      $cache: '@cache.customer'
```
...
- **Decorator pattern** - `CachedCustomerRepository` wraps the persistence-specific repository
...
- **Graceful fallback** to database on cache errors
- **API Platform compatibility** via `__call()` magic method (when `framework.api_platform` is set)
