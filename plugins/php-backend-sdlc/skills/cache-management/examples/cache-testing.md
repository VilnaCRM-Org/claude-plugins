...
The `Customer` entity is illustrative; namespaces follow the PSR-4 prefix of `architecture.source_root`. Tests that bypass the repository use Doctrine's `ObjectManager` so they work for any `persistence.mapper` — fetch the manager service per the mapper:

| `persistence.mapper` | Object manager service id                |
| -------------------- | ---------------------------------------- |
...
└── Integration/
    └── Customer/
        └── Infrastructure/
            └── Repository/
                ├── CachedCustomerRepositoryCacheTest.php
                ├── CachedCustomerRepositoryInvalidationTest.php
                └── CachedCustomerRepositorySwrTest.php
```
...
final class CachedCustomerRepositoryInvalidationTest extends KernelTestCase
{
...
final class CachedCustomerRepositoryCacheTest extends KernelTestCase
{
    private CustomerRepositoryInterface $repository;
...
final class CustomerRepositoryTtlTest extends KernelTestCase
{
    ...
    /**
     * Update customer directly via the object manager (bypass cache invalidation).
     * Service id depends on persistence.mapper — see the table at the top of this guide.
     */
    private function updateCustomerNameDirectly(string $customerId, string $newName): void
    {
        $om = $this->objectManager();

        $customer = $om->find(Customer::class, $customerId);
...
final class CachedCustomerRepositorySwrTest extends KernelTestCase
{
    private CustomerRepositoryInterface $repository;
    private TagAwareCacheInterface $cache;
    ...
    /**
     * Note: SWR behavior is handled by the cache contracts' probabilistic early
     * expiration (beta parameter). This test verifies basic caching behavior;
     * full SWR requires a message bus for background refresh.
     */
...
Run the target mapped by `make.tests` (skip with a capability-absent note when it is `null`). Where the Makefile passes arguments through, filter to the cache group; the exact invocation is repository-specific:
...
vendor/bin/phpunit tests/Integration/Customer/Infrastructure/Repository/CachedCustomerRepositoryCacheTest.php
