...
The examples below use an illustrative "Customer" resource with Type and
Status dependencies. Substitute the resources of your own service's
bounded contexts (`architecture.bounded_contexts`). IRI-based patterns
(`@id`, `hydra:member`) apply when `framework.api_platform` is set;
GraphQL helpers apply when `framework.graphql` is true.
...
Create reusable validation logic (the `hydra:member` collection format
is API Platform's — applies when `framework.api_platform` is set):
