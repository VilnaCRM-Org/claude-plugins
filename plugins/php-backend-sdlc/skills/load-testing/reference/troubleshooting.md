...
2. **Reduce VU Count**:
   ...
   Start with the smoke-level variant of the target mapped by
   `make.load_tests` (2-5 VUs):
...
1. Verify the database container (engine declared in
   `persistence.engine`) is running:

   ```bash
   docker compose ps
   ```

2. Check the connection string — it must match `persistence.engine`:

   ```bash
   echo $DB_URL
   ```

   Canonical upstream value (MongoDB engine):

   ```bash # profile-example
   # DB_URL should be: mongodb://localhost:27017/test
   ```

3. Test database connectivity with the console command matching
   `persistence.mapper` (Doctrine ORM or ODM). Canonical upstream
   (ODM):
...
2. Manual cleanup if needed — reset the test database via the
   project's database reset target (discover it in the Makefile).
   Canonical upstream:
...
2. Review this skill's [SKILL.md](../SKILL.md) and reference files
3. Examine working examples in the harness scripts directory (`tests/Load/scripts/`)
4. Check the project-specific README in the load-test harness directory
