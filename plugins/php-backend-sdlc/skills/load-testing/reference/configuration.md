...
All load-test invocations go through the target mapped by
`make.load_tests` in the project profile (gated by
`capabilities.load_testing`). Level-specific variants shown below are
repository conventions paired with that target — discover them in the
repository Makefile.
...
# Database (for setup/teardown) — connection string for the engine
# declared in persistence.engine
DB_URL=<engine-specific connection string>
```
...
Start with smoke, then increase. Use the level-specific variants of the
target mapped by `make.load_tests`; canonical upstream layout:
