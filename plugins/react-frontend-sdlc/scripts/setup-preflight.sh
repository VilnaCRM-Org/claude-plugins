#!/usr/bin/env bash
# setup-preflight.sh — environment preflight for /fe-sdlc-setup
# (FR-2, NFR-7, ADR-10).
#
...
# where /fe-sdlc-setup runs `bmalph init` right after preflight), a
# JavaScript package manager (any one of bun, pnpm, or npm — the consumer
# repos differ; do NOT hard-require bun), Node.js >= 20.0.0 (the lowest
# engines.node floor across the consumer repos; do NOT require 24), docker
# (presence required; a running daemon is optional at setup time — several
# lanes boot it on demand), a YAML toolchain (yq, or python3 with PyYAML —
# ADR-2), and a JSON toolchain (jq or python3) that ai-review-loop.sh,
# fr-nfr-gate.sh, get-pr-comments.sh and generate-profile.sh hard-depend on
# for parsing claude/gh JSON — without it those scripts fail mid-loop with a
# misleading diagnosis instead of at setup.
#
# Default mode aborts on the FIRST failing check, printing its named
# remediation, so /fe-sdlc-setup can halt early with one actionable error.
...
# there is nothing to examine yet: /fe-sdlc-setup runs `bmalph init` right
...
# --- 8. Node.js >= 20.0.0 -----------------------------------------------------
# Node remains the runtime even where bun manages dependencies. The floor is
# the lowest engines.node across the consumer repos (some pin >= 24, others
# >= 20); requiring 24 here would wrongly reject a Node-20 repo at setup.
check_version "node" "$FLOOR_NODE" \
  "install or upgrade Node.js to >= $FLOOR_NODE: https://nodejs.org (use .nvmrc / nvm to match the repo's pinned version)" \
  node --version

# --- 9. docker -----------------------------------------------------------------
# The dev/test lanes run inside Docker (the configured make targets compose
# dev/prod plus the rca / jscpd services). The daemon need not be running at
# setup time — several lanes start it on demand — so a stopped daemon is a
# note in the detail column, not a failure; only a missing binary fails.
if ! command -v docker >/dev/null 2>&1; then
  record FAIL "docker" "'docker' not found on PATH" \
    "install Docker and ensure the daemon can start: https://docs.docker.com/get-docker/"
elif docker info >/dev/null 2>&1; then
  record PASS "docker" "docker present, daemon running" "-"
else
  record PASS "docker" "docker present (daemon not running — start it before container-based lanes)" "-"
fi

# --- 10. YAML toolchain (ADR-2) ------------------------------------------------
if have_yq; then
...
# --- 11. JSON toolchain (jq or python3) ----------------------------------------
