...
REVIEW_PROMPT="${REVIEW_PROMPT:-Review the working tree changes of this repository (git diff against ${DIFF_BASE}) for correctness, accessibility, maintainability, FR/NFR coverage, and code health: component design tradeoffs, appropriate React/component pattern use, code smells, SOLID/DRY/KISS, bulletproof-react modular architecture, and repository rules. Keep findings concrete and scoped to changed code or directly affected behavior. Apply safe fixes directly. End your response with the mandatory verdict line: 'AI_REVIEW_VERDICT: PASS' if the changes are acceptable, otherwise 'AI_REVIEW_VERDICT: FAIL'.}"
