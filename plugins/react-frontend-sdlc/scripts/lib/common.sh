#!/usr/bin/env bash
# common.sh — shared helpers for react-frontend-sdlc plugin scripts.
#
...
log_info()  { printf '[react-sdlc][INFO] %s\n' "$*"; }
log_warn()  { printf '[react-sdlc][WARN] %s\n' "$*" >&2; }
log_error() { printf '[react-sdlc][ERROR] %s\n' "$*" >&2; }
...
SDLC_PROFILE_RELPATH=".claude/react-sdlc.yml"
