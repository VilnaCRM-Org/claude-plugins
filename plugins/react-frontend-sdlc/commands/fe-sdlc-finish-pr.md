---
description: "Create or update the PR, then drive the CI-fix and comment-resolution loops until checks are green and zero AI review comments remain unresolved"
argument-hint: "[slug | PR-URL]"
---

# /fe-sdlc-finish-pr — PR finishing loops (FR-8)
...
- The PR: the `slug | PR-URL` argument, or the current branch's PR
  (`gh pr view`); if none exists yet, step 1 creates it. A `PR-URL`
  resolves the PR directly; a `slug` names the `specs/<slug>/` planning
  chain and the PR is resolved from the current branch.
...
- Profile keys: `ci.provider` and `ci.required_checks` (CI loop and its
  ...
  `make.pr_comments` (unresolved-thread source; script substitutes when
  `null`), `make.ai_review_loop` (degraded comment source; script
  substitutes when `null`).

## Procedure

1. **PR create/update** — first resolve the PR and its state with
   `gh pr view --json state,url,number` (or `gh pr view <PR-URL>` when a
   PR-URL argument is given; a `slug` argument selects the
   `specs/<slug>/` chain and the PR is still the current branch's):
...
2. **CI fix loop — counter A** — degrade check first: if the profile's
   ...
   CAUSE in the working tree — never suppressing findings (no
   `eslint-disable` / `@ts-ignore`), never editing quality thresholds
   (governance-protected), never softening a workflow. The agent runs NO
   git and returns exactly one of its four contract statuses
...
3. **Comment source selection** — if `review.coderabbit` is true (or
   ...
   when the key is `null`) each pass and treats its findings as the
   comment set to resolve (NFR-4). Never run the AI review loop directly
   from this command: the agent is the single owner of the degrade
   source, so the loop runs exactly once per iteration and every fix it
   applies stays inside counter B's accounting. Record the substitution
   as a degrade note in the final report and name the selected source in
   the step-4 dispatch prompt.
...
4. **Comment resolution loop — counter B** — dispatch the
   ...
   unresolved is the comment-listing command resolved from
   `make.pr_comments` (the bundled script substitutes when the mapping
   is `null`):
...
exist, and the agent-run AI review loop substitutes as the comment
