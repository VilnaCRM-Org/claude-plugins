---
description: "Run the BMAD planning chain non-interactively for a GitHub issue (or a task description), writing the six-artifact specs/<slug>/ chain until readiness passes"
argument-hint: "[issue-URL | task-description]"
---

# /fe-sdlc-plan — issue → planning artifacts (FR-4)
...
- The issue or task: the URL argument, the `ISSUE_URL:` line emitted by
  `/fe-sdlc-issue`, or — when `/fe-sdlc-plan` is invoked directly with a
  free-text task-description instead of a URL — that description. When a
  URL is supplied, resolve it with
  ...
  closed or unresolvable issue. A bare task-description skips the `gh`
  resolution and seeds the chain directly.
- `<slug>`: kebab-case of the issue title (prefix with the issue
  number, e.g. `42-currency-selector`), or of the task-description when
  no issue is given; all artifacts live under `specs/<slug>/`.

## Procedure

1. Resolve the issue (or accept the task-description) and derive
   `<slug>` as above.
...
4. The chain writes the six artifacts under `specs/<slug>/`, in order:
   ...
   5. `epics-stories.md` — epics and stories, each story marked
      independent or dependent (the `/fe-sdlc-implement`
      parallel-dispatch input)
