---
description: "Run the full frontend SDLC loop end-to-end: setup-check, issue, plan, implement, review, qa, finish-pr — gated transitions, resumable, loop-safe"
argument-hint: "[task-description | issue-URL]"
---

# /fe-sdlc — end-to-end orchestrator (FR-1)
...
| --- | ----------- | ------------------------------ | -------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| 0   | setup-check | profile + preflight validation | valid `.claude/react-sdlc.yml`, preflight fresh                                  | halt → instruct `/fe-sdlc-setup` (never auto-generate profile in-loop) |
| 1   | issue       | `/fe-sdlc-issue`               | GitHub issue URL exists with testable AC                                         | 5 iterations                                                           |
| 2   | plan        | `/fe-sdlc-plan`                | `specs/` chain complete, readiness PASS                                          | 5 iterations                                                           |
| 3   | implement   | `/fe-sdlc-implement`           | Ralph `EXIT_SIGNAL` success, all stories done                                    | 5 iterations + circuit breaker                                         |
| 4   | review      | `/fe-sdlc-review`              | zero new findings in last gate iteration; a11y gate clean                        | 5 iterations                                                           |
| 5   | qa          | `/fe-sdlc-qa`                  | QA verdict PASS — visual + E2E + Lighthouse + a11y (FAIL routes back to stage 3) | 5 iterations                                                           |
| 6   | finish-pr   | `/fe-sdlc-finish-pr`           | CI green + 0 unresolved AI review comments                                       | 5 iterations                                                           |
...
and stage 1 itself (`/fe-sdlc-issue`) repeats this dedup search before
creating, so a duplicate cannot be opened even on a racing re-run.

| Observed state                                                                        | Resume at                              |
| ------------------------------------------------------------------------------------- | -------------------------------------- |
| profile invalid/missing or preflight stale                                            | stage 0 → HALT: "run `/fe-sdlc-setup`" |
| no managed open issue (`gh issue list --label react-frontend-sdlc` matching the task) | stage 1                                |
...
| stories done; no zero-findings review-gate record (or a11y gate not clean)            | stage 4                                |
...
HALTS the run with the instruction to run `/fe-sdlc-setup`. This command
...
3. **Gate check**: after the stage reports done, VERIFY its exit
   ...
   `readiness.md`, re-run the review/a11y gate, re-run `gh pr checks`) —
   a stage's own success claim is not the gate; the verified condition
   is.
...
| review | <n>/5 | zero new findings in last gate iteration; a11y gate clean | yes/no |
| qa | <n>/5 (loop-backs: <k>) | QA verdict PASS — visual + E2E + Lighthouse + a11y | yes/no |
