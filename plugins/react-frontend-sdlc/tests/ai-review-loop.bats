...
#
# Single-shot verdict cases use the shared stub claude
...
#
# This is the react-frontend-sdlc suite: the loop logs under the
# [react-sdlc] prefix and resolves its profile from .claude/react-sdlc.yml.
...
@test "log lines carry the [react-sdlc] prefix" {
  STUB_CLAUDE_OUTPUT="$PASS_JSON" STUB_CLAUDE_LOG="$CALLS" run "$LOOP"
  [ "$status" -eq 0 ]
  [[ "$output" == *"[react-sdlc]"* ]]
}
...
@test "R2 Bug 10: a malformed profile yields one clean diagnostic, no raw traceback" {
  ...
  # .claude/react-sdlc.yml fails with a [react-sdlc] message naming the file
