...
setup() {
  ...
  mkdir -p "$REPO"
  git -C "$REPO" init -q
  BEGIN='<!-- react-frontend-sdlc:begin -->'
  END='<!-- react-frontend-sdlc:end -->'
}
...
@test "missing files are created holding exactly one block" {
  ...
  for f in CLAUDE.md AGENTS.md; do
    ...
    grep -q 'Make-map execution' "$REPO/$f"
  done
}
...
@test "CRLF managed block is recognized: replaced in place, no duplicate (IG-E5)" {
  ...
  run "$INJECT" "$REPO"
  [ "$status" -eq 0 ]
  [ "$(grep -c 'react-frontend-sdlc:begin' "$REPO/CLAUDE.md")" -eq 1 ]
...
@test "block text references profile keys, not hardcoded values" {
  run "$INJECT" "$REPO"
  grep -q '`.claude/react-sdlc.yml`' "$REPO/CLAUDE.md"
