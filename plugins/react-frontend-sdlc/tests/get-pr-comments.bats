#!/usr/bin/env bats
# Tests for scripts/get-pr-comments.sh (FR-8 feed, ADR-4).
#
...
@test "full listing: both threads, issue comment, summary line" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"PR #7"* ]]
  [[ "$output" == *"[unresolved] src/modules/alpha/features/login/login-form.tsx:42"* ]]
  [[ "$output" == *"[resolved] src/modules/beta/components/menu.tsx:17"* ]]
...
@test "--unresolved-only: resolved thread and issue comments dropped" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"[unresolved] src/modules/alpha/features/login/login-form.tsx:42"* ]]
  [[ "$output" != *"Rename this component"* ]]
  [[ "$output" != *resolved\]\ src/modules/beta* ]]
  [[ "$output" != *"ci-bot"* ]]
  [[ "$output" == *"unresolved threads: 1"* ]]
}
...
assert c["path"] == "src/modules/alpha/features/login/login-form.tsx"
assert c["line"] == 42
...
# bare jq exit 5 / python traceback and ZERO [react-sdlc] diagnostic (the
...
@test "R3 Bug 6 regression: a healthy multi-comment payload still renders (both backends)" {
  GOOD='{"data":{"repository":{"pullRequest":{"reviewThreads":{"pageInfo":{"hasNextPage":false},"nodes":[{"isResolved":false,"comments":{"pageInfo":{"hasNextPage":false},"nodes":[{"author":{"login":"alice"},"body":"x","path":"src/a.tsx","line":1,"url":"u1"},{"author":{"login":"carol"},"body":"y","path":"src/b.tsx","line":2,"url":"u2"}]}}]},"comments":{"pageInfo":{"hasNextPage":false},"nodes":[{"author":{"login":"bob"},"body":"lgtm","url":"u3"}]}}}}}'
