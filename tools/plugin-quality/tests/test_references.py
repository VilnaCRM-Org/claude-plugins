...
class _Tree:
    ...
    def __init__(self) -> None:
        ...
        _write(
            self.root / "skills" / "testing-workflow" / "SKILL.md",
            "# testing-workflow\n",
        )
        ...
        _write(
            self.root / "agents" / "code-quality-reviewer.md",
...
class ReferenceCheckTests(unittest.TestCase):
    ...
    def test_rf1_script_negative_typo(self):
        self.tree.skill(
            "code-review",
            "# code-review\n\nRun "
            "`${CLAUDE_PLUGIN_ROOT}/scripts/validate-profle.sh`.\n",
        )
...
class RealPluginCleanTest(unittest.TestCase):
    def test_real_plugin_has_no_dead_references(self):
        ...
        self.assertEqual(
            [],
            findings,
            "real plugin should be clean; over-matching regex: "
