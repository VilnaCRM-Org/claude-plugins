...
class FrontmatterTestBase(unittest.TestCase):
    ...
    def write_command(self, name: str, text: str) -> None:
        (self.plugin_root / "commands" / f"{name}.md").write_text(
            text, encoding="utf-8"
        )
    ...
    def assert_no_findings(self, findings):
        self.assertEqual(
            findings,
            [],
            msg=f"expected no findings, got: {[f.render() for f in findings]}",
        )
    ...
    def assert_finding(self, findings, check, rule, msg_substr):
        matches = [
            ...
            if f.check == check
            and f.rule == rule
            and msg_substr.lower() in f.message.lower()
        ]
        self.assertTrue(
            matches,
            msg=(
...
class TestFM2(FrontmatterTestBase):
    ...
    def test_negative_bare_string_not_array(self):
        ...
        self.assert_finding(
            self.run_check(),
            "L2",
            "frontmatter.command.allowed-tools",
...
class TestFM3(FrontmatterTestBase):
    ...
    def test_negative_missing_model(self):
        ...
        self.assert_finding(
            self.run_check(), "L3", "frontmatter.agent.missing-key", "model"
        )
...
class TestFM4(FrontmatterTestBase):
    ...
    def test_negative_declares_model(self):
        ...
        self.assert_finding(
            self.run_check(), "L4", "frontmatter.skill.shape", "must not declare model"
        )
...
class TestParseError(FrontmatterTestBase):
    def test_malformed_frontmatter_surfaced(self):
        ...
        self.assert_finding(
            self.run_check(), "L0", "frontmatter.parse-error", "failed to parse"
        )
