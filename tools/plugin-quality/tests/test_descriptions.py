...
class DescriptionsTest(unittest.TestCase):
    ...
    def _skill(self, name, **fm):
        ...
        (d / "SKILL.md").write_text(_fm(name=name, **fm) + f"\n# {name}\n", encoding="utf-8")
    ...
    def test_ds2_p_use_when_passes(self):
        self._skill("alpha", description="Builds the CRUD endpoints. Use when adding API resources.")
    ...
    def test_ds2_e_when_to_use_phrase_passes(self):
        self._skill("alpha", description="When to use this skill: adding API resources to the project.")
        self.assertNotIn("descriptions.skill.no-trigger", self._rules_for("alpha"))

    # ---- DS-3 (L13) agent delegation trigger ----------------------------

    def test_ds3_p_delegate_passes(self):
        self._agent("rev", description="Delegate to this agent when reviewing changes for quality.")
    ...
    def test_ds3_e_proactively_passes(self):
        self._agent("rev", description="Proactively reviews diffs for silent failures and bad error handling.")
        self.assertNotIn("descriptions.agent.no-trigger", self._rules_for("rev.md"))

    # ---- DS-4 (L14) non-empty / >= 20 chars -----------------------------

    def test_ds4_p_normal_passes(self):
        self._command("run", description="Run the suite over a pull request branch.", **{"argument-hint": "[pr]"})
    ...
    def test_clean_tree_has_no_findings(self):
        self._skill("alpha", description="Builds CRUD endpoints. Use when adding API resources.")
        self._agent("rev", description="Delegate to this agent when reviewing diffs for quality.")
        self._command("run", description="Run the quality suite over a PR branch.", **{"argument-hint": "[pr]"})
