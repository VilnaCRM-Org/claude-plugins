...
class Dimension:
    ...
    block_floor: int = (
        2  # CI hard-block: a *critical* dim blocks only at score <= block_floor
    )
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.block_floor, int) or isinstance(self.block_floor, bool):
            raise ValueError(
                f"{self.id}: block_floor must be an int, got {self.block_floor!r}"
            )
...
DIMENSIONS: tuple[Dimension, ...] = (
    Dimension(
        ...
        guidance=(
            ...
            "Reward a concrete 'Use when ... / Skip when <flag> / NOT for X (use Y "
            "instead)' shape that discriminates against sibling components. Penalize "
            "vague triggers ('helps with APIs', 'code reviewer') that would over- or "
            "under-trigger. "
    ...
    Dimension(
        ...
        floor=4,
        guidance=(
            "Does the body deliver exactly what the `description` promises — no "
            "over-claim, no under-claim, and NO self-contradiction (e.g. description "
            "promises caching but body says 'this skill does NOT cover caching')? "
            "Score 1 on any direct contradiction between the description and the body."
        ),
    ),
    Dimension(
        id="J3",
        ...
        floor=4,
        guidance=(
            "For every external-capability or missing-input dependency, does the "
            "described degrade path actually TERMINATE — never loop forever, never "
            "silently hard-fail? "
            "It must end in an explicit reported outcome (e.g. SUCCESS-WITH-REPORT, "
            "SKIPPED, ESCALATED). Score 1 if any degrade path loops ('retry until it "
            "appears') or just crashes with no defined behavior. Reward citing NFR-4 "
            "and a concrete fallback "
    ...
    Dimension(
        ...
        floor=4,
        guidance=(
            "Is the command's single exit condition stated consistently and "
            "measurably across its "
            "own sections (Procedure, Loop & exit condition, Iteration guard) and "
            "consistent with "
            "the FR id named in its H1 — not vague, not contradicting itself between "
            "sections? "
            "Penalize an exit condition that is phrased one way in one section and a "
            "materially "
            "different way in another, or that is too vague to verify. Judge INTERNAL "
            "consistency "
            "only; do not require an external stage table the artifact does not "
            "contain."
        ),
    ),
    Dimension(
        ...
        floor=4,
        guidance=(
            "Does the prose loop logic actually BOUND iterations (MAX_ITERATIONS=5), "
            "restate the "
            "counter each turn, and explicitly NEVER auto-reset a tripped circuit "
            "breaker? "
            "Penalize any instruction to reset/clear a breaker and retry, or a loop "
            "with no "
    ...
    Dimension(
        ...
        floor=4,
        guidance=(
            "For every profile key the skill lists under '## Profile keys consumed', "
            "does the body "
            "actually BRANCH behavior on it where relevant, with BOTH conditional "
            "branches present "
            "(e.g. persistence.mapper -> both ORM-migration and ODM-schema paths; "
            "engine -> both "
            "MySQL EXPLAIN and Mongo explain())? Penalize a key that is listed but "
            "never used, or a "
    ...
    Dimension(
        ...
        floor=4,
        guidance=(
            "Beyond the literal denylist, is there PARAPHRASED or STRUCTURAL leakage "
            "of the source "
            "project (a user/OAuth service backed by MongoDB)? E.g. 'the Mongo-backed "
            "repository that "
            "stores users', a hardcoded assumption that the persistence engine is "
            "MongoDB, an example "
            "that only makes sense for a user-account domain. The component must read "
            "as generic to ANY "
            "PHP backend. Score 1 on a clear domain assumption baked in as if "
            "universal; score 5 when "
    ...
    Dimension(
        ...
        floor=4,
        guidance=(
            "Do the instructions steer toward ROOT-CAUSE fixes and explicitly forbid "
            "suppressing "
            "findings, lowering quality thresholds, or editing config (deptrac.yaml, "
            "baselines) to "
            "pass? Penalize any loophole that permits 'add a baseline entry' / 'relax "
            "the threshold' / "
    ...
    Dimension(
        ...
        name_filter="qa",
        guidance=(
            "QA components only: are verdicts derived STRICTLY from observed runtime "
            "behavior "
            "(HTTP responses, CLI output) and never from reading implementation "
            "source? Penalize any "
    ...
    Dimension(
        ...
        floor=4,
        guidance=(
            "Does the guide enumerate EXACTLY the skills that ship in the plugin — no "
            "stale entries, "
            "no missing ones — and does every decision-tree branch point to a real "
            "skill whose "
            "description matches the branch condition? You will be given the "
            "authoritative list of "
            "shipped skill names; score 1 if the guide lists a skill not in that set "
            "or omits one."
        ),
    ),
    Dimension(
        ...
        guidance=(
            ...
            "('validate the profile or skip if needed' — when?)? Penalize ambiguous "
            "conditionals, "
