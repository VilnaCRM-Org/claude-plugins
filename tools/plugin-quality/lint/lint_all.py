...
def run(
    plugin_roots: list[pathlib.Path], repo_root: pathlib.Path
) -> list[_common.Finding]:
    ...
    for root in plugin_roots:
        for mod in modules:
            try:
                ...
                Exception
            ) as exc:  # a crashing check is itself a finding, never a silent pass
                findings.append(
                    _common.Finding(
                        check="ERR",
...
def _emit(
    args, findings: list[_common.Finding], plugin_roots: list[pathlib.Path]
) -> int:
    ...
    if args.json:
        ...
        print(
            ...
            f"(S1={counts['S1']} S2={counts['S2']} "
            f"S3={counts['S3']} S4={counts['S4']})",
            file=sys.stderr,
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Static prompt-quality lint over plugin trees."
    )
    parser.add_argument(
        "paths",
        nargs="*",
        help="plugin roots (default: all plugins/*/ with a manifest)",
    )
    parser.add_argument("--json", action="store_true", help="emit JSON")
    parser.add_argument(
        "--repo-root", default=None, help="override repo root (for marketplace check)"
    )
    ...
    if not plugin_roots:
        ...
        print(
            f"No plugin roots found under {repo_root}/plugins/ — nothing to lint.",
            file=sys.stderr,
        )
        return 0
