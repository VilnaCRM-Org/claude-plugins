    ...
    if fm_name != stem:
        findings.append(
            Finding(
                ...
                severity="S2",
                path=art.rel,
                message=(f"agent name {fm_name!r} must equal filename stem {stem!r}"),
            )
        )
...
def _check_skill_name(art: "_model.Artifact") -> list[Finding]:
    ...
    return [
        Finding(
            ...
            path=art.rel,
            message=(
                f"skill name {fm_name!r} must equal parent directory name {dir_name!r}"
            ),
        )
    ]
...
def _check_model_enum(art: "_model.Artifact") -> list[Finding]:
    ...
    return [
        Finding(
            ...
            severity="S2",
            path=art.rel,
            message=(f"model {model_str!r} not in {sorted(MODEL_ENUM)}"),
        )
    ]
