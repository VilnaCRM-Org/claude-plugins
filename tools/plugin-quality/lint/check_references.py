...
_SKILLPATH_RE = re.compile(
    r"\$\{CLAUDE_PLUGIN_ROOT\}/skills/([A-Za-z0-9_*-]+)/SKILL\.md"
)
...
def _rule_link(ctx: _Ctx, art) -> None:
    """L21 — relative .md links resolve from the linking file's dir."""
    for target in dict.fromkeys(_LINK_RE.findall(art.raw)):
        ...
        if not (art.path.parent / cleaned).resolve().is_file():
            ctx.add(
                ...
                art,
                f"]({target})",
                f"dead relative link: {cleaned} does not resolve "
                f"from {art.path.parent.name}/",
            )
